...
Description:
A collection of functions to more easily facilitate printing to the Brother QL
label printers without having to memorize the ESC/P commands. Also handles
sending to sockets for you.
"""
...
class BrotherPrint(object):
    ...
    def select_charset(self, charset):
        """Select international character set and changes codes in code table
        accordingly.
    ...
    def select_char_code_table(self, table):
        ...
        Args:
            table: The desired character code table. Choose from 'standard',
                   'eastern european', 'western european', and 'spare'.
    ...
    def cut_setting(self, cut):
        ...
        Args:
            cut: The type of cut setting we want. Choices are 'full', 'half',
                 'chain', and 'special'.
    ...
    def rotated_printing(self, action):
        ...
        Args:
            action: The desired printing orientation. 'rotate' enables rotated
                    printing. 'normal' disables rotated printing.
    ...
    def feed_amount(self, amount):
        ...
        Args:
            amount: the form feed setting you desire. Options are '1/8', '1/6',
                    'x/180', and 'x/60', with x being your own desired amount.
                    X must be a minimum of 24 for 'x/180' and 8 for 'x/60'
    ...
    def page_length(self, length):
        """Specifies page length. This command is only valid with continuous
        length labels.
        ...
        if length < 12000:
            self.send(
                chr(27) + '(' + 'C' + chr(2) + chr(0) + chr(mL) + chr(mH)
            )
        else:
            raise RuntimeError('Length must be less than 12000.')

    def page_format(self, topmargin, bottommargin):
        """Specify settings for top and bottom margins. Physically printable
        area depends on media.

        Args:
            topmargin: the top margin, in dots. The top margin must be less
                       than the bottom margin.
            bottommargin: the bottom margin, in dots. The bottom margin must be
                          less than the top margin.
        ...
        if (tL+tH*256) < (BL + BH*256):
            self.send(
                chr(27) + '(' + 'c' + chr(4) + chr(0) + chr(tL) + chr(tH) +
                chr(BL) + chr(BH)
            )
        else:
            raise RuntimeError(
                'The top margin must be less than the bottom margin'
            )
    ...
    def left_margin(self, margin):
        ...
        Args:
            margin: The left margin, in character width. Must be less than the
                    media's width.
    ...
    def right_margin(self, margin):
        ...
        Args:
            margin: The right margin, in character width, must be less than the
                    media's width.
        ...
        if margin >= 1 and margin <= 255:
            ...
            raise RuntimeError(
                'Invalid margin parameter in function rightMargin'
            )

    def horz_tab_pos(self, positions):
        """Sets tab positions, up to a maximum of 32 positions. Also can clear
        tab positions.

        Args:
            positions: either a list of tab positions (between 1 and 255),
                       or 'clear'.
        ...
        if positions.min < 1 or positions.max > 255:
            raise RuntimeError(
                'Invalid positions parameter in function horz_tab_pos'
            )
        sendstr = chr(27) + 'D'
        if len(positions) < 32:
            ...
            raise RuntimeError('Too many positions in function horz_tab_pos')
    ...
    def vert_tab_pos(self, positions):
        ...
        if positions.min < 1 or positions.max > 255:
            raise RuntimeError(
                'Invalid positions parameter in function vert_tab_pos'
            )
        sendstr = chr(27) + 'D'
        if len(positions) <= 16:
            for position in positions:
            ...
            raise RuntimeError('Too many positions in function vert_tab_pos')
    ...
    def forward_feed(self, amount):
        """Calling this function finishes input of the current line, then moves
        the vertical print position forward by x/300 inch.

        Args:
            amount: how far foward you want the position moved. Actual movement
                    is calculated as amount/300 inches.
        Returns:
            None
        ...
        if amount <= 255 and amount >= 0:
            ...
            raise RuntimeError(
                'Invalid foward feed, must be less than 255 and >= 0'
            )
    ...
    def abs_vert_pos(self, amount):
        ...
        Args:
            amount: The distance from the top margin you'd like,
                    from 0 to 32767
        ...
        if amount < 32767 and amount > 0:
            self.send(
                chr(27) + '(' + 'V' + chr(2) + chr(0) + chr(mL) + chr(mH)
            )
        else:
            raise RuntimeError(
                'Invalid vertical position in function abs_vert_pos'
            )

    def abs_horz_pos(self, amount):
        """Calling this function sets the absoulte print position for the next
        data, this is the position from the left margin.

        Args:
            amount: desired positioning. Can be a number from 0 to 2362.
                    The actual positioning is calculated as (amount/60)inches
        ...
        n2 = amount // 256
        self.send(
            chr(27) + '${n1}{n2}'.format(n1=chr(n1), n2=chr(n2))
        )

    def rel_horz_pos(self, amount):
        """Calling this function sets the relative horizontal position for the
        next data, this is the position from the current position. The next
        character will be printed (x/180)inches away from the current position.
        The relative position CANNOT be specified to the left.
        This command is only valid with left alignment.

        Args:
            amount: desired positioning. Can be a number from 0 to 7086.
                    The actual positioning is calculated as (amount/180)inches
    ...
    def alignment(self, align):
        ...
        Args:
            align: desired alignment. Options are 'left', 'center', 'right',
                   and 'justified'. Anything else will throw an error.
    ...
    def carriage_return(self):
        """Performs a line feed amount, sets next print position to the
        beginning of the next line, will ignore a subsequent line feed command.
    ...
    def line_feed(self):
        """Performs line feed operation, any carriage return command subsequent
        to a lineFeed will be ignored.
    ...
    def frame(self, action):
        ...
        if action in choices:
            ...
            raise RuntimeError(
                'Invalid action for function frame, choices are on and off'
            )
    ...
    def bold(self, action):
        ...
        Args:
            action: Enable or disable bold printing.
        ...
        if action == 'on':
            ...
            raise RuntimeError(
                'Invalid action for function bold. '
                'Options are on and off.'
            )
    ...
    def italic(self, action):
        ...
        Args:
            action: Enable or disable italic printing.
                    Options are 'on' and 'off'.
        ...
        if action == 'on':
            ...
            raise RuntimeError(
                'Invalid action for function italic. '
                'Options are on and off.'
            )
        self.send(chr(27) + action)
    ...
    def double_strike(self, action):
        ...
        Args:
            action: Enable or disable doublestrike printing.
                    Options are 'on' and 'off'.
        ...
        if action == 'on':
            ...
            raise RuntimeError(
                'Invalid action for function doubleStrike. '
                'Options are on and off.'
            )
        self.send(chr(27) + action)
    ...
    def double_width(self, action):
        ...
        Args:
            action: Enable or disable doublewidth printing.
                    Options are 'on' and 'off'.
        ...
        if action == 'on':
            ...
            raise RuntimeError(
                'Invalid action for function doubleWidth. '
                'Options are on and off'
            )
    ...
    def compressed_char(self, action):
        ...
        Args:
            action: Enable or disable compressed character printing.
                    Options are 'on' and 'off'.
        ...
        if action == 'on':
            ...
            raise RuntimeError(
                'Invalid action for function compressedChar. '
                'Options are on and off'
            )
    ...
    def underline(self, action):
        ...
        Args:
            action: Enable or disable underline printing.
                    Options are '1' - '4' and 'cancel'
    ...
    def char_size(self, size):
        ...
        Args:
            size: change font size.
                  Options are 24' '32' '48' for bitmap fonts
                  33, 38, 42, 46, 50, 58, 67, 75, 83, 92, 100, 117, 133, 150,
                  167, 200 233, 11, 44, 77, 111, 144 for outline fonts.
        ...
        Raises:
            ...
            Warning: Your font is currently set to outline and you have
                     selected a bitmap only font size
            Warning: Your font is currently set to bitmap and you have selected
                     an outline only font size
        """
        sizes = {
            '24': 0,
            '32': 0,
        ...
        if size in sizes:
            if (
                size in ['24', '32', '48'] and
                self.fonttype != self.font_types['bitmap']
            ):
                raise Warning(
                    'Your font is currently set to outline and '
                    'you have selected a bitmap only font size'
                )
            if (
                size not in ['24', '32', '48'] and
            ...
            self.send(
                chr(27) + 'X' + chr(0) + chr(int(size)) + chr(sizes[size])
            )
        else:
            raise RuntimeError(
                'Invalid size for function charSize, '
                'choices are auto 4pt 6pt 9pt 12pt 18pt and 24pt'
            )
    ...
    def select_font(self, font):
        ...
        if font in fonts:
            if font in [
                'broughham', 'lettergothicbold', 'brusselsbit', 'helsinkibit',
                'sandiego'
            ]:
                self.fonttype = self.font_types['bitmap']
            else:
    ...
    def char_style(self, style):
        ...
        Args:
            style: The desired character style.
                   Choose from 'normal', 'outline', 'shadow',
                   and 'outlineshadow'
    ...
    def barcode(
        self, data, format, characters='off', height=48, width='small',
        parentheses='on', ratio='3:1', equalize='off', rss_symbol='rss14std',
        horiz_char_rss=2
    ):
        ...
        Args:
            ...
            format: the barcode type you want. Choose between code39, itf,
                    ean8/upca, upce, codabar, code128, gs1-128, rss
            characters: Whether you want characters below the bar code.
                        Chose between 'off' and 'on'.
            height: Height, in dots.
            width: width of barcode.
                   Choose 'xsmall' 'small' 'medium' 'large'
            parentheses: Parentheses deletion 'on' or 'off'.
                         'on' or 'off' only matters with GS1-128.
            ratio: ratio between thick and thin bars.
                   Choose '3:1', '2.5:1', and '2:1'
            equalize: equalize bar lengths, choose 'off' or 'on'
            rss_symbol: rss symbols model, choose from 'rss14std', 'rss14trun',
                        'rss14stacked', 'rss14stackedomni', 'rsslimited',
                        'rssexpandedstd', 'rssexpandedstacked'
            horiz_char_rss: for rss expanded stacked, specify the number of
                            horizontal characters, must be an even number
                            b/w 2 and 20.
        """
        ...
        n1 = height % 256
        if (
            format in barcodes and
            width in widths and
            ratio in ratios and
            characters in character_choices and
            rss_symbol in rss_symbols
        ):
            sendstr += (
                chr(27) + 'i' + 't' + barcodes[format] + 's' + 'p' + 'r' +
                character_choices[characters] + 'u' + 'x' + 'y' + 'h' +
                chr(n1) + chr(n2) + 'w' + widths[width] + 'e' +
                parentheses_choices[parentheses] + 'o' +
                rss_symbols[rss_symbol] + 'c' + chr(horiz_char_rss) + 'z' +
                ratios[ratio] + 'f' + equalize_choices[equalize] + 'b' +
                data + chr(92)
            )
    ...
    def set_number_of_copies(self, copies):
        ...
        if copies in range(1, 999):
            ...
            raise RuntimeError(
                'Invalid number of copies. Number must be between 1 and 999'
            )

    def select_priority_print_option(self, action):
        """ Default is 0 - Priority to print speed.
        Set to 1 for priority to print quality.
        ...
        if action == 'on':
            ...
            raise RuntimeError(
                'Invalid action for function select_priority_print_option'
            )
        self.send('^QS'+action)
